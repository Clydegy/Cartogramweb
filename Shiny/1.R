...
my_Map_polys_df <- SpatialPolygonsDataFrame(my_Map_polys,
                                            data.frame(UN = pw_dat_code$FIPS.Code,
  ...
  output$map <- renderLeaflet({
    ...
    leaflet() %>% setView(0, 0, zoom=1) %>%
      addPolygons(
        ...
        popup = with(my_Map_polys_df@data, htmltools::htmlEscape(sprintf("%s: %s", NAME, index())))
      )
  })
  ...
  output$map2 <- renderLeaflet({
    ...
    leaflet() %>% setView(0, 0, zoom=1) %>%
      addPolygons(
        ...
        color = "#000", weight = 1, opacity = 0.5,
        fillColor = ~colorNumeric(pal, index())(index()), fillOpacity = 1, 
        popup = with(my_Carto_polys_df@data, htmltools::htmlEscape(sprintf("%s: %s", NAME, index())))
      )
  })
