...
void init_gridv (void)
{
  ...
  for (i=0; i<lx-1; i++) {
	...
	- rho_ft[(i+1)*ly + j] /
	(PIE * ((di+1)/dlx + (j/(di+1)) * (j/dly) * (dlx/dly)));
  }
  for (j=0; j<ly; j++)
  ...
  for (i=0; i<lx; i++) {
	...
	-rho_ft[i*ly + j + 1] /
	(PIE * ((di/(j+1)) * (di/dlx) * (dly/dlx) + (j+1)/dly));
  }
  for (i=0; i<lx; i++)
...
#pragma omp parallel for
    for (k=0; k<lx*ly; k++) {
      ...
      vx_intp[k] = interpol(xproj[k], yproj[k], gridvx, (char*) "x");
      vy_intp[k] = interpol(xproj[k], yproj[k], gridvy, (char*) "y");
    }
    accept = false;
    while (!accept) {
