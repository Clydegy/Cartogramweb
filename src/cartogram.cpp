...
void project (bool proj_graticule)
{
  ...
  for (i=0; i<n_poly; i++)
    for (j=0; j<n_polycorn[i]; j++) {
      cartcorn[i][j].x =
	interpol(polycorn[i][j].x, polycorn[i][j].y, xdisp, (char*) "x")
	+ polycorn[i][j].x;
      cartcorn[i][j].y =
	interpol(polycorn[i][j].x, polycorn[i][j].y, ydisp, (char*) "y")
	+ polycorn[i][j].y;
    }
  if (proj_graticule) {
    ...
    for (i=0; i<lx*ly; i++) {
      x2 = xproj2[i];
      y2 = yproj2[i];
      xproj2[i] = interpol(x2, y2, xdisp, (char*) "x") + x2;
      yproj2[i] = interpol(x2, y2, ydisp, (char*) "y") + y2;	
    }
  }
...
void output_to_ascii (void)
{
  ...
  for (i=0; i<n_reg; i++) 
    for (j=0; j<n_polyinreg[i]; j++)
      cart_area[i] += 
	polygon_area(n_polycorn[polyinreg[i][j]], cartcorn[polyinreg[i][j]]);
